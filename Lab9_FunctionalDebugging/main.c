...
// Initializes the Port F pins for I/O
...
{	
  ...
  EnableInterrupts();
	...
	unsigned long now = 0;
	unsigned long switchStatus = 0;
	unsigned long data;
	unsigned long prevData;
	
  while (1) {
		switchStatus = GPIO_PORTF_DATA_R & 0x11;
		
		if (switchStatus == 0x0011) {
...
{
	// 50 ms
	unsigned long volatile time = 72727;
